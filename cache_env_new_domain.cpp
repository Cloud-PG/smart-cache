...
class WindowElement{
    public:
        ...
        vector<float> concat_with_next_values(float occupancy, float hit_rate){
            ...
            for(int i = 4; i < 128 + 12 + 4; i++) next_values.push_back(cur_values[3]);
...
class Stats {
    public:
        ...
        inline FileStats get_or_set(int filename, float size, int datatype, int request, int campaign){
            ...
            if(found == false){
                ...
                for(int i=0; i<128; i++) {
                    if(i==campaign) stats.campaigns[i] = 1;
                    else stats.campaigns[i] = 1;
                }
                for(int i=0; i<12; i++) stats.sites[i] = 0;
...
class cache {
    public: 
        ...
        inline FileStats before_request(int filename, bool hit, float size, int datatype, int request, int campaign, int sitename){
            FileStats stats;
            stats = _stats.get_or_set(filename, size, datatype, request, campaign);  
            stats.sites[sitename] += 1;
            if (hit == true) 
                stats._hit += 1;
            else
                stats._miss += 1;     
...
class env{
    public:
        ...
        void purge();
        void set_curValues_add(float size, float frequency, float recency, float datatype, float occupancy, float hit_rate, int campaign, int sitename);
        void set_curValues_evict(float size, float frequency, float recency, float datatype, float occupancy, float hit_rate);
...
void env::set_curValues_add(float size, float frequency, float recency, float datatype, float occupancy, float hit_rate, int campaign, int sitename){
    _curValues.clear();
    ...
    for(int i = 0; i<128; i++){
        if(i==campaign) _curValues.push_back(1.);
        else _curValues.push_back(0.);
    }
    for(int i = 0; i<12; i++){
        if(i==sitename) _curValues.push_back(1.);
        else _curValues.push_back(0.);
    }
...
void env::set_curValues_evict(float size, float frequency, float recency, float datatype,  float occupancy, float hit_rate){
    int filename = get_filename_from_cache(_cached_files_index);
    FileStats filestats = get_stats(filename);
    _curValues.clear();
    _curValues.push_back(size);
    _curValues.push_back(frequency);
    _curValues.push_back(recency);
    _curValues.push_back(datatype);
    for(int i = 0; i<(128); i++) _curValues.push_back(filestats.campaigns[i]);
    for(int i = 0; i<(12); i++) _curValues.push_back(filestats.sites[i]);
    _curValues.push_back(occupancy);
    _curValues.push_back(hit_rate);
}
...
PYBIND11_MODULE(cache_env_cpp_domain, m) {
    ...
    py::class_<env>(m, "env")
        ...
        .def("set_curValues_add", &env::set_curValues_add)
        .def("set_curValues_evict", &env::set_curValues_evict)
