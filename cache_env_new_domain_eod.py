...
class FileStats(object):
    ...
    def __init__(self, size: float, datatype: int, campain, site):
        ...
        self._sites = dict.fromkeys(range(0,it_total_sites),0)
        self._sites[site] += 1
...
class env:
    ...
    def __init__(self, start_month, end_month, directory, out_directory, out_name, time_span, purge_delta, output_activation):
        ...
        self._request_window_elements = set()
        self._eviction_window_elements = set()
        ...
        for i in range(0,it_total_sites):
            one_hot_site[i] = filestats._sites[i]
    ...
    def update_windows(self, adding_or_evicting, curValues, nextValues, action):
        ''' add this adding/evicting action to corresponding window '''

        if adding_or_evicting == 0:
            #self._request_window_elements.append(WindowElement(1, curValues, 0, action, nextValues))
            self._request_window_elements |= set([WindowElement(1, curValues, 0, action, nextValues)])
        
        elif adding_or_evicting == 1:
            #self._eviction_window_elements.append(WindowElement(1, curValues, 0, action, nextValues))
            self._eviction_window_elements |= set([WindowElement(1, curValues, 0, action, nextValues)])

    def give_reward(self):
        ''' give rewards to all elements in windows, put them in memory and clear windows'''
        
        if self._cache._dailyReadOnHit > self._cache._dailyReadOnMiss * 0.3:
            multiplier = 1.
        else:
            multiplier = -1.
        
        #for i in range(len(self._request_window_elements)):
        #    obj = self._request_window_elements[i]
        if self._output_activation == 0:
            for obj in self._request_window_elements:
                ...
                obj.reward = multiplier * coeff
                to_add = obj.concat()
                self.add_memory_vector = np.vstack((self.add_memory_vector, to_add))
            
            #for i in range(len(self._eviction_window_elements)): 
            #    obj = self._eviction_window_elements[i]
            for obj in self._eviction_window_elements:
                size = obj.cur_values[0]
                if size <= it_liminf_size:
                    coeff = 0 
                elif size >= it_limsup_size:
        ...
        self._request_window_elements.clear()          
