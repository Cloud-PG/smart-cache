...
python -m SmartCache.sim.simulator sim ../../datasets/HighFrequencyDataset/ --force-exe-build 'true' --cache-type lru,lfu,sizeSmall,sizeBig --cache-size 10 --cache-size-unit "G" --simulation-steps normal --window-start 0 --window-stop 52 --region it --cache-size 10 --cache-bandwidth 1 --cache-bandwidth-redirect 'true' --out-folder ../../results/sim_syntethic_dataset/HighFrequencyDataset &&
