...
if args.region == 'it':
    environment = cache_env_cpp_domain.env(
...
def get_next_request_values():    
    ...
    campain = dataframe_.df.Campain[environment._curRequest]
    sitename = dataframe_.df.SiteName[environment._curRequest]
    
    filestats = environment._cache.before_request(
        filename, hit, size, datatype, environment._curRequest_from_start, campain, sitename)
    
    environment.set_curValues_add(filestats._size/1000,filestats._hit + filestats._miss, environment._curRequest_from_start - filestats._last_request,0. if filestats._datatype == 0 else 1., environment._cache.capacity()/100.,environment._cache.hit_rate(), campain, sitename)
...
def get_next_file_in_cache_values():
    ...
    environment.set_curValues_evict(filestats._size/1000,filestats._hit + filestats._miss, environment._curRequest_from_start - filestats._last_request,0. if filestats._datatype == 0 else 1., environment._cache.capacity()/100.,environment._cache.hit_rate())
