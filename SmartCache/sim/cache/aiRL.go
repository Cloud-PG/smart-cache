	...
	if cache.evictionTableOK {
		...
		if expEvictionTradeoff < cache.evictionTable.Epsilon {
			...
			if cache.qEvictionPrevState.Action != 0 && len(cache.qEvictionPrevState.State) != 0 {
				...
				if !hit || cache.qEvictionPrevState.HitRate > cache.HitRate() || cache.dataReadOnHit <= cache.dataReadOnMiss || cache.dataReadOnMiss >= (cache.bandwidth*0.5) {
					reward = -reward
				}
				// Update table
		...
		if expAdditionTradeoff > cache.additionTable.Epsilon {
			...
			if !hit {
				...
				if curAction == qlearn.ActionStore {
					...
					// 	reward = -reward
					// }
					if fileStats.Frequency == 1 && (cache.dataWritten >= (cache.dataReadOnHit*0.3) || cache.dataReadOnMiss >= (cache.bandwidth*0.5)) {
						reward = -reward
					}
					// Update table
					cache.additionTable.Update(curState, curAction, reward)
