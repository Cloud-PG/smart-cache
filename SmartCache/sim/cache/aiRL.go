	...
	if cache.evictionTableOK {
		...
		if expEvictionTradeoff < cache.evictionTable.Epsilon {
			...
			if cache.qEvictionPrevState.Action != 0 && len(cache.qEvictionPrevState.State) != 0 {
				...
				if !hit || cache.qEvictionPrevState.HitRate > cache.HitRate() || cache.dataReadOnHit <= cache.dataReadOnMiss || cache.dataReadOnMiss >= (cache.bandwidth*0.95) {
					reward = -reward
				}
				// Update table
		...
		if expAdditionTradeoff > cache.additionTable.Epsilon {
			...
			if !hit {
				...
				if inPrevStates { // Some action are not taken randomly
					...
					// 	reward = -reward
					// }
					if cache.qEvictionPrevState.HitRate > cache.HitRate() || cache.dataReadOnHit <= cache.dataReadOnMiss || cache.dataReadOnMiss >= (cache.bandwidth*0.95) {
						reward = -reward
					}
