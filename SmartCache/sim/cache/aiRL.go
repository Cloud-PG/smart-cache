		...
		if expAdditionTradeoff > cache.additionTable.Epsilon {
			...
			if !hit {
				...
				if curAction == qlearn.ActionNotStore {
					...
					// 	reward = -reward
					// }
					if fileStats.Frequency > 1 && cache.dataReadOnHit <= cache.dataReadOnMiss && cache.dataReadOnMiss <= (cache.bandwidth*0.75) {
						reward = -reward
					}
				...
				if curAction == qlearn.ActionStore {
					...
					// 	reward = -reward
					// }
					if fileStats.Frequency == 1 && cache.dataWritten >= cache.dataReadOnHit && cache.dataReadOnMiss >= (cache.bandwidth*0.75) {
						reward = -reward
					}
					// Update table
					cache.additionTable.Update(curState, curAction, reward)
