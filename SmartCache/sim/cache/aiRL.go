	...
	if cache.evictionTableOK {
		...
		if expEvictionTradeoff < cache.evictionTable.Epsilon {
			...
			if cache.qEvictionPrevState.Action != 0 && len(cache.qEvictionPrevState.State) != 0 {
				...
				if cache.qEvictionPrevState.HitRate > cache.HitRate() || cache.dataReadOnHit <= (cache.dataReadOnMiss*0.5) || cache.dataReadOnMiss >= (cache.bandwidth*0.75) {
					reward = -reward
				}
				// Update table
		...
		if expAdditionTradeoff > cache.additionTable.Epsilon {
			...
			if !hit {
				...
				if inPrevStates { // Some action are not taken randomly
					...
					if fileStats.Frequency > 1 && cache.dataReadOnHit <= (cache.dataReadOnMiss*0.5) {
						reward = -reward
					}
				...
				if curAction == qlearn.ActionNotStore {
					...
					// 	reward = -reward
					// }
					if fileStats.Frequency > 1 && (cache.dataReadOnHit <= (cache.dataReadOnMiss*0.5) || cache.dataReadOnMiss <= (cache.bandwidth*0.75)) {
						reward = -reward
					}
