...
import math
import random
import cache_env_new_domain_eod
#import cache_env_new_domain_us
import csv
import array
import os
import argparse
...
model_evict.add(Dense(256, input_dim= input_len))
...
if use_target_model == True:
    ...
    target_model_evict.add(Dense(256, input_dim= input_len))
...
model_add.add(Dense(256,input_dim = input_len))
...
if use_target_model == True:
    ...
    target_model_add.add(Dense(256,input_dim = input_len))
...
###### START LOOPING ############################################################################################################################
if args.region == 'it':
    environment = cache_env_new_domain_eod.env(
        _startMonth, _endMonth, data_directory, out_directory, out_name, time_span, purge_delta, output_activation)

elif args.region == 'us':
    environment = cache_env_new_domain_eod_us.env(
...
while end == False:
    ...
    if adding_or_evicting == 0:
        ...
        if step_add % 1000 == 0:
            print('Request: ' + str(environment.curRequest + 1) + ' / ' + str(environment.df_length) + '  -  Occupancy: ' + str(round(environment._cache.capacity,2)) 
                + '%  -  ' + 'Hit rate: ' + str(round(environment._cache._hit/(environment._cache._hit + environment._cache._miss)*100,2)) +'%' + ' ACTION: ' +  str(action))
            print('add window:' + str(len(environment._request_window_elements)))
            print('evict window:' + str(len(environment._eviction_window_elements)))
            print('add memory:' + str(len(environment.add_memory_vector)))
            print('evict memory:' + str(len(environment.evict_memory_vector)))
            print()      
        # IF DAY IS OVER, GIVE REWARD TO ALL DAILY ACTIONS
        if (environment.curRequest + 1) == environment.df_length:
            environment.give_reward()
        
        #PURGE UNUSED STATS
        if (environment.curDay+1) % purge_frequency == 0 and environment.curRequest == 0:
            environment.purge()
        
        # IF ADDING IS NOT OVER, GET NEXT VALUES AND ADD ACTION TO WINDOW
        if environment._cache.capacity <= environment._cache._h_watermark:
            next_values = environment.get_next_request_values()
            if anomalous == False:
                if environment._cache._dailyReadOnMiss / DailyBandwidth1Gbit * 100 < 95. or hit == True:
                    environment.update_windows(adding_or_evicting, cur_values, next_values, action)
        
        # REMOVE THE FIRST DUMMY ELEMENT IN MEMORY
        if step_add == 1000:
            environment.add_memory_vector = np.delete(environment.add_memory_vector, 0, 0)

        # KEEP MEMORY LENGTH LESS THAN LIMIT BOTH FOR ADD AND EVICT
    ...
    ### EVICTING #############################################################################################################
        ...
        # IF EVICTING IS NOT OVER, GET NEXT VALUES AND ADD ACTION TO WINDOW
