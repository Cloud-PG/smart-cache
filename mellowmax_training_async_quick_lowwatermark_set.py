...
###### START LOOPING ############################################################################################################################
environment = cache_env_async_quick_set.env(_startMonth, _endMonth, data_directory, out_directory, out_name, time_span, purge_delta)
random.seed(seed_)
...
while end == False:
    ...
    ### EVICTING #############################################################################################################
        ...
        if action == 1:
            ...
            environment._cache._filesLRU.remove(curFilename)
